...
from wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, \
	screen_coords, isBlackPatch, isWhitePatch, to_percentiles, isWhitePatch_S, isBlackPatch_S
import itertools
...
class WholeSlideImage(object):
	...
	def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
		...
		save_path_hdf5 = os.path.join(save_path, str(self.name.split("/")[-1]) + '.h5')
		# save_path_hdf5 = save_path
		
		print(f"process_contours函数  {save_path_hdf5}")
	...
	def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size=256, step_size=256,
		...
		pool.close()
		results = np.array([result for result in results if
		                    result is not None])
		
		print('Extracted {} coordinates'.format(len(results)))
		pool = mp.Pool(num_workers)

		iterable1 = [(coord, patch_level, patch_size, self.path) for coord in results]
		results = pool.starmap(WholeSlideImage.filter_block, iterable1)
		pool.close()
		results = np.array([result for result in results if result is not None])
		print('Extracted {} coordinates by filter'.format(len(results)))
	...
	@staticmethod
	def filter_block(coord, patch_level, patch_size, file_path):
		wsi = openslide.open_slide(file_path)
		
		patch = wsi.read_region((coord[0], coord[1]), patch_level, (patch_size, patch_size)).convert('RGB')
		
		if isWhitePatch(np.array(patch), 15) or isWhitePatch_S(np.array(patch),210,0.5)or isBlackPatch_S(np.array(patch)):
			coord = None
		wsi.close()
		return coord
