...
def patching(WSI_object, **kwargs):
	...
	print(f"patching函数  {kwargs['save_path']}")
	# Patch
...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir,
	...
	for i in range(total):
		...
		slide_id= slide_name[:-4]

		if auto_skip and os.path.isfile(os.path.join(patch_save_dir, slide_id + '.h5')):
			print('{} already exist in destination location, skipped'.format(slide_id))
			df.loc[idx, 'status'] = 'already_exist'
			continue
		if not args.tcga_flag:
			# Inialize WIS
			slide_dir = os.path.join(source, slide)
			real_slide = os.listdir(slide_dir)
			slide_name = None
			for s in real_slide:
				if os.path.isfile(os.path.join(slide_dir, s)) and s.split(".")[-1] == "svs":
					slide_name = s
					print(f"读取目录 {slide_dir}下的文件 {slide_name}")

			if slide_name is None:
				miss_cnt += 1
				print(f"目录{slide_dir}内无slide")
				continue
		...
		if patch:
			print(f"seg_and_patch函数  {patch_save_dir}")
			#注释处 控制是否在40x放大下缩放成20x的patch
			# if int(df.loc[idx, 'aperio.AppMag']) == 40:
			# 	patch_size = 512
			# 	step_size = 512
		...
		if stitch:
			file_path = os.path.join(patch_save_dir, slide_name[:-4] + '.h5')
			if os.path.isfile(file_path):
				heatmap, stitch_time_elapsed = stitching(file_path, WSI_object, downscale=64)
				stitch_path = os.path.join(stitch_save_dir,  slide_name[:-4]  + '.jpg')
...
parser.add_argument('--process_list', type=str, default='process_list.csv',
...
if __name__ == '__main__':
	...
	seg_params = {'seg_level': -1, 'sthresh': 8, 'mthresh': 7, 'close': 4, 'use_otsu': True,
				  'keep_ids': 'none', 'exclude_ids': 'none'}
	filter_params = {'a_t': 100, 'a_h': 3, 'max_n_holes': 2048}
